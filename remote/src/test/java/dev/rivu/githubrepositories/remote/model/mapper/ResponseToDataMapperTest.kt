...
class ResponseToDataMapperTest {
    ...
    fun `mapFromDomain should return same data`() {
        ...
        val dataBuiltBy = data.builtBy!!
        for (i in dataBuiltBy.indices) {
            assertEquals(dataBuiltBy[i].avatar, mappedData.builtBy[i].avatar)
            assertEquals(dataBuiltBy[i].href, mappedData.builtBy[i].href)
            assertEquals(dataBuiltBy[i].username, mappedData.builtBy[i].username)
        }
    }
    ...
    fun `mapToDomain should return same data`() {
        ...
        assertEquals(data.currentPeriodStars, mappedData.currentPeriodStars)
        assertEquals(data.builtBy.size, mappedData.builtBy?.size)
        val mappedDataBuiltBy = mappedData.builtBy!!
        for (i in data.builtBy.indices) {
            assertEquals(data.builtBy[i].avatar, mappedDataBuiltBy[i].avatar)
            assertEquals(data.builtBy[i].href, mappedDataBuiltBy[i].href)
            assertEquals(data.builtBy[i].username, mappedDataBuiltBy[i].username)
        }
    }
}